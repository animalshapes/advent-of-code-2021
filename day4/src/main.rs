...
fn calculate_board_score(board_nums: &HashSet<u32>, draws: Vec<u32>) -> u32 {
    let draws_set = draws.clone().into_iter().collect::<HashSet<_>>();
...
fn main() {
    ...
    let board_nums = boards
        .iter()
        .map(|board| board.clone().into_iter().flatten().collect::<HashSet<_>>())
        .collect::<Vec<_>>();
    ...
    let draws_sequence = (board_size..draws.len())
        .map(|index| draws.clone().into_iter().take(index).collect::<Vec<_>>())
        .collect::<Vec<_>>();

    for draw in draws_sequence {
        let draw_set = draw.clone().into_iter().collect::<HashSet<_>>();

        let bingos = board_combos
            .iter()
            .zip(board_nums.iter())
            .filter(|(board_combo, _)| {
                let draw_set_copy = draw_set.clone().into_iter().collect::<HashSet<_>>();
                check_board(&board_combo[..], draw_set_copy)})
            .map(|(_, nums)| nums)
            .collect::<Vec<_>>();
